...
using MoireIVC.LL_HF: LLHFNumPara, LLHFSysPara
using MoireIVC.LL_HF: polar_azimuthal_angles, berry_curvature, realspace_pauli
using MoireIVC.Basics: ql_cross


export LLHF_plot_band_3D, LLHF_plot_band, LLHF_plot_phi


phase_color = range(Makie.Colors.HSV(0,1,1), stop=Makie.Colors.HSV(360,1,1), length=90);


function LLHF_plot_band_3D(rho; para::LLHFNumPara)

    N1 = para.N1; N2 = para.N2
    G1 = para.system.G1; G2 = para.system.G2

    bands = LL_HF.band(rho)
    ...
    GLMakie.activate!()
    surface(plot_x, plot_y, bands[1,:,:]./W0; 
        axis=(;type=Axis3, aspect=(sqrt(3),1,1)),
    )
    surface!(ax, plot_x, plot_y, bands[2,:,:]./W0;
        #=colormap = phase_color,
        colorrange = (-π, π),
        color = [ angle(rho[1+k1, 1+k2,1,1,2,1])
            for k1 in 0:N1-1, k2 in 0:N2-1
        ],=#
    )
...
function LLHF_plot_band(rho; para::LLHFNumPara, unit=:meV)
    ...
    bands = LL_HF.band(rho)
    b1 = diag(bands[1,:,:])./u
    b1 = [b1; bands[1,1:N1÷2+1,1]./u]
    b2 = diag(bands[2,:,:])./u
    b2 = [b2; bands[2,1:N1÷2+1,1]./u]
...
#=
function plot_realspace(ρ; moduleis = IVC,
    a = moduleis.a_Moire, N = 36, xlimits = (-1.2, 1.2), 
    ylimits = (-1.2, 1.2), 
    arrowscale = 1, arrowsize = 4, colormap = :berlin, colorrange = nothing,
    ...
    for (i, j) in Iterators.product(eachindex(x1_range), eachindex(y1_range))
        ...
        plot_x[index] = x
        plot_y[index] = y
        plot_Sx[index], plot_Sy[index], plot_Sz[index] = realspace_pauli(x*a, y*a, ρ)[[1,2,3]]
        index += 1
    end
    ...
    primitive_cell ./= moduleis.a_Moire
...
function LLHF_plot_DM_phase(ρ; para::LLHFNumPara,
    ...
    al = [ angle(ρ[1+(k1+N1)%N1, 1+(k2+N2)%N2,1,1,2,1])
...
function LLHF_plot_phi(ρ; para::LLHFNumPara)
    ...
    al = [ angle(ρ[1+k1-floor(Int64,k1//N1)*N1, 1+k2-floor(Int64,k2//N2)*N2,1,1,2,1])
    ...
    Colorbar(fig[1,2], colormap = phasecolor, colorrange = (-pi,pi),
...
function plot_periodic_BZ(matrix; N1 = N1, N2 = N2, bc=false,
