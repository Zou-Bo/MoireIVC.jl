...
# H[τn′, τn, k1, k2] * c†_{k,τn′}c_{k,τn}
# ρ[τ, τ′, k1, k2] = <c†_{k,τn′}c_{k,τn}>
begin
    ...
    @kwdef mutable struct LLHFNumPara
        ...
        # H[τn′, τn, k1, k2] * c†_{k,τn′}c_{k,τn}
        H0::Array{ComplexF64,4} = zeros(ComplexF64, 2, 2, N1, N2)
        ...
        # ρ[τ, τ′, k1, k2] = <c†_{k,τn′}c_{k,τn}>
        DMseed::Array{ComplexF64,4} = fill(ComplexF64(0.5), 2, 2, N1, N2)
    ...
    function H0_C3_T!(num_para, gap=0.1)
        num_para.H0 .= gap * [num_para.system.W0 * sqrt(4/27) *
            ...
            (1.5-τn)*(τn′==τn)
                for τn′ in 1:2, τn in 1:2, k1 in 0:num_para.N1-1, k2 in 0:num_para.N2-1
        ];
        return num_para
    end
...
function hf_interaction(ρ, para::LLHFNumPara, part::Symbol)
    ...
    elseif part == :XE
        ...
        error("part can only be :HA, :HE, :XA, :XE")
    end
    return para.system.W0 / para.k_num * H
end
"Tr[ρ*O] expectation value: ρ[τ, τ′, k1, k2] * O[τ′, τ, k1, k2]"
function trace(rho, O)
    #return @tensor rho[τ, τ′, k1, k2] * O[τ′, τ, k1, k2]
    return @reduce sum(τ,τ′,k1,k2) rho[τ,τ′,k1,k2] * O[τ′,τ,k1,k2]
end
"Energy Per Area"
...
module CrystalSym
    ...
    function (Ts::Trans)(rho)
        ...
        for k1 in 0:N1-1, k2 in 0:N2-1
            ...
            rho[1,2,1+k1,1+k2] = (rho[1,2,1+k1,1+k2] + rho[1,2,1+Tk1,1+Tk2]*cis( phase)) / 2.
            rho[2,1,1+k1,1+k2] = (rho[2,1,1+k1,1+k2] + rho[2,1,1+Tk1,1+Tk2]*cis(-phase)) / 2.
            rho[1,1,1+k1,1+k2] = (rho[1,1,1+k1,1+k2] + rho[1,1,1+Tk1,1+Tk2] ) / 2.
            rho[2,2,1+k1,1+k2] = (rho[2,2,1+k1,1+k2] + rho[2,2,1+Tk1,1+Tk2] ) / 2.

            rho[1,2,1+Tk1,1+Tk2] = rho[1,2,1+k1,1+k2] * cis(-phase)
            rho[2,1,1+Tk1,1+Tk2] = rho[2,1,1+k1,1+k2] * cis(phase)
            rho[1,1,1+Tk1,1+Tk2] = rho[1,1,1+k1,1+k2]
            rho[2,2,1+Tk1,1+Tk2] = rho[2,2,1+k1,1+k2]
        end
    end
    ...
    function (C3::Rot3)(rho)
        ...
        for k1 in 0:N1-1, k2 in 0:N2-1
            ...
            rho[1,2,1+k1,1+k2] = (rho[1,2,1+k1,1+k2] + rho[1,2,1+Rk1,1+Rk2]*cis( phaseR) + rho[1,2,1+R2k1,1+R2k2]*cis( phaseR2)) / 3.
            rho[2,1,1+k1,1+k2] = (rho[2,1,1+k1,1+k2] + rho[2,1,1+Rk1,1+Rk2]*cis(-phaseR) + rho[2,1,1+R2k1,1+R2k2]*cis(-phaseR2)) / 3.
            rho[1,1,1+k1,1+k2] = (rho[1,1,1+k1,1+k2] + rho[1,1,1+Rk1,1+Rk2]              + rho[1,1,1+R2k1,1+R2k2]              ) / 3.
            rho[2,2,1+k1,1+k2] = (rho[2,2,1+k1,1+k2] + rho[2,2,1+Rk1,1+Rk2]              + rho[2,2,1+R2k1,1+R2k2]              ) / 3.

            rho[1,2,1+Rk1,1+Rk2] = rho[1,2,1+k1,1+k2] * cis(-phaseR)
            rho[2,1,1+Rk1,1+Rk2] = rho[2,1,1+k1,1+k2] * cis( phaseR)
            rho[1,1,1+Rk1,1+Rk2] = rho[1,1,1+k1,1+k2]
            rho[2,2,1+Rk1,1+Rk2] = rho[2,2,1+k1,1+k2]

            rho[1,2,1+R2k1,1+R2k2] = rho[1,2,1+k1,1+k2] * cis(-phaseR2)
            rho[2,1,1+R2k1,1+R2k2] = rho[2,1,1+k1,1+k2] * cis( phaseR2)
            rho[1,1,1+R2k1,1+R2k2] = rho[1,1,1+k1,1+k2]
            rho[2,2,1+R2k1,1+R2k2] = rho[2,2,1+k1,1+k2]
        end
    end
    ...
    function (Inv::PT)(rho)
        ...
        for k1 in 0:N1-1, k2 in 0:N2-1
            ...
            rho[1,2,1+k1,1+k2] = (rho[1,2,1+k1,1+k2] + rho[1,2,1+Mk1,1+Mk2]*cis( phase) ) / 2.
            rho[2,1,1+k1,1+k2] = (rho[2,1,1+k1,1+k2] + rho[2,1,1+Mk1,1+Mk2]*cis(-phase) ) / 2.

            rho[1,2,1+Mk1,1+Mk2] = rho[1,2,1+k1,1+k2] * cis(-phase)
            rho[2,1,1+Mk1,1+Mk2] = rho[2,1,1+k1,1+k2] * cis( phase)

            if Inv.sym == :T
                rho[1,1,1+k1,1+k2] = (rho[1,1,1+k1,1+k2] + rho[2,2,1+Mk1,1+Mk2] )/2.
                rho[2,2,1+k1,1+k2] = (rho[2,2,1+k1,1+k2] + rho[1,1,1+Mk1,1+Mk2] )/2.
                rho[1,1,1+Mk1,1+Mk2] = rho[2,2,1+k1,1+k2]
                rho[2,2,1+Mk1,1+Mk2] = rho[1,1,1+k1,1+k2]
            elseif Inv.sym == :P
                rho[1,1,1+k1,1+k2] = (rho[1,1,1+k1,1+k2] + rho[1,1,1+Mk1,1+Mk2] )/2.
                rho[2,2,1+k1,1+k2] = (rho[2,2,1+k1,1+k2] + rho[2,2,1+Mk1,1+Mk2] )/2.
                rho[1,1,1+Mk1,1+Mk2] = rho[1,1,1+k1,1+k2]
                rho[2,2,1+Mk1,1+Mk2] = rho[2,2,1+k1,1+k2]
            elseif Inv.sym == :PT
                rho[1,1,1+k1,1+k2] = 0.5
                rho[2,2,1+k1,1+k2] = 0.5
                rho[1,1,1+Mk1,1+Mk2] = 0.5
                rho[2,2,1+Mk1,1+Mk2] = 0.5
            else
...
#=
    # old implementation of hartree-fock self-consistent loop
    ...
    function hf_onestep!(new_rho, ρ; para::LLHFNumPara,
        ...
        for k1 in axes(H,3), k2 in axes(H,4)
    ...
    function LLHF_solve_old(para, ρ = copy(para.DMseed);
        coherence = 0.0,
...
function add_phi!(rho, phi = angle(rho[1,2,1,1]))
    rho[1, 2, :,:] .*= cis(-phi)
    rho[2, 1, :,:] .*= cis(+phi)
    return rho
end
function LLHF_solve(para, ρ = copy(para.DMseed);
    ...
    function EPA(ρ, Hint = hf_interaction(ρ, para))
        LLHF_EnergyPerArea(ρ; para=para, Hint=Hint)
    end
    function HFINT(ρ)
...
function realspace_pauli(rx, ry, ρ, para::LLHFNumPara)
    ...
    for τ in 1:2, τ′ in 1:2
        for k1 in 1:N1, k2 in 1:N2
            local_density[τ,τ′] += ρ[τ,τ′,k1,k2] * WF[k1,k2,τ] * conj(WF[k1,k2,τ′])
        end
    end
